...
#define CRIT_TEMP_ENTER         (550)
#define CRIT_TEMP_ZONE		(530)	/*2 degrees buffer for timing related issues from modem*/
...
/* 2012-12-18, jikhwan.jeong@lge.com, [V7][Power][Common] Add battery_charging_current kernel debug log. [END] */
	...
	int type_of_charger = USB_CHG_TYPE__INVALID;
...
/* 2012-12-18, jikhwan.jeong@lge.com, [V7][Power][Common] Add battery_charging_current kernel debug log. [END] */
	...
	if( (battery_temp > CRIT_TEMP_ZONE) && (CHARGER_TYPE_NONE == charger_type) ) {
		type_of_charger = get_charger_type();
		printk("Charger Type %d \n",type_of_charger);
		if (type_of_charger != USB_CHG_TYPE__INVALID){
			/*Because of High temperature charger type is coming as Zero (No charger) from modem.
			  From Usb driver we can get proper chrger type */
...
// LGE_CHANGE_S,narasimha.chikka@lge.com, Add Full charging condition
		...
		else if((charger_status == CHARGER_STATUS_BAD)&&(msm_batt_info.current_chg_source!=0))
		{
...
// LGE_CHANGE_S,narasimha.chikka@lge.com, Add Full charging condition
			...
			if (battery_capacity >= 100) {
				DBG_LIMIT("BATT: FULL \n");
				msm_batt_info.batt_status = POWER_SUPPLY_STATUS_FULL;
			} else if ( (battery_temp >= CRIT_TEMP_ENTER) && (type_of_charger != USB_CHG_TYPE__INVALID)) {
				/*Probably charging stopped from modem side*/
				DBG_LIMIT("BATT: High Temp Discharge \n");
...
// LGE_CHANGE_E,narasimha.chikka@lge.com, Add Full charging condition
	...
	if (supp) {
		...
		if(supp!=&msm_psy_batt)
			power_supply_changed(&msm_psy_batt);//update battery status/voltage change  uevent to  framework.
	}
	pr_info("%s: exit\n", __func__);
